# Frequently asked questions

## Is MacBaram open source?

No. MacBaram is proprietary, closed-source software. This repository contains public documentation and update notes only.

## Where should I download MacBaram?

Only from [https://www.macbaram.com/download](https://www.macbaram.com/download). GitHub Releases is not a MacBaram installer channel.

## Does MacBaram support Intel Macs?

No. Apple silicon and macOS 13 or later are required.

## Does MacBaram support iMac?

iMac support is not currently declared. When the required capability is not verified, MacBaram keeps the affected control unavailable.

## Will MacBaram always improve performance or extend battery life?

No. MacBaram provides controls and visibility. Results depend on the Mac, workload, environment, settings, and macOS behavior.

## Why are prices not listed here?

Pricing, trial terms, and availability are commercial information maintained on the [official website](https://www.macbaram.com/#pricing). Keeping one commercial source prevents stale or conflicting copies.

## Where are release notes?

User-visible changes appear in [CHANGELOG.md](../CHANGELOG.md) only after the corresponding release has been promoted to the official download channel and verified there.
